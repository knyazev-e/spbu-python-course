...
if "m1" not in globals():
    m1: List[List[float]] = [[1]]
if "m2" not in globals():
    m2: List[List[float]] = [[1]]


if not m or all(len(row) == 0 for row in m):
    raise ValueError("The matrix is empty.")
elif any(len(m[0]) != len(row) for row in m):
    raise ValueError("A matrix must have a constant number of elements in each row.")
else:
...
if (
    ...
    matrix_multiplication_result: List[List[float]] = [
        [sum(m1[i][j] * m2[j][k] for j in range(len(m1[0]))) for k in range(len(m2[0]))]
